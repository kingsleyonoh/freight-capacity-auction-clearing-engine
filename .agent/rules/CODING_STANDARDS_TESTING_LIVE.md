...
- Your database (local PostgreSQL, local Redis, local DuckDB replay store, MiniZinc/OR-Tools solver process (optional live solver)) — validates schema, column names, constraints, query behavior
...
> This section applies to Dream API, server-rendered HTMX UI, workers, and CLI tools. If the project has a server-rendered HTMX frontend, use the Component Testing section above instead.
...
- Use the framework's built-in test client (e.g., Dream request handlers, local HTTP tests, or project integration helpers)
...
- Name: `module_name_test.ml` — co-located or in `tests/` mirror
