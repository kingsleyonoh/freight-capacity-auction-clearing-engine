# Freight Capacity Auction Clearing Engine — Coding Standards: E2E Testing (Real Endpoints)
...
tests/e2e/
  api/                        ← Backend E2E tests
    auth.e2e.ml or auth.e2e.test.ts          ← Auth endpoint tests
    auctions.e2e.test.ts   ← Simulation endpoint tests
...
| Run command | `dune runtest --no-buffer && dune exec tests/integration/main.exe && npx playwright test` | `npx playwright test` |
