...
- **Do not treat workflow read instructions as automatic full-file reads.** Read the smallest sufficient section first: headings, matching task entry, referenced PRD section, nearby source, or the specific rule section that applies.
- Read an entire file only when it is small, you will substantially edit/rewrite it, the task is broad/cross-cutting/safety-sensitive, or targeted reading leaves uncertainty.
...
- For source files, read the whole file when modifying it heavily; otherwise read the relevant function/module and surrounding context.
