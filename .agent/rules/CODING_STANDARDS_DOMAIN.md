# Freight Capacity Auction Clearing Engine — Coding Standards: Domain & Production
...
2. Tests run against local services (local PostgreSQL, local Redis, local DuckDB replay store, MiniZinc/OR-Tools solver process (optional live solver))
...
1. **All tests pass** — `dune runtest --no-buffer && dune exec tests/integration/main.exe && npx playwright test` / equivalent shows 0 failures
...
## Freight Auction Clearing Domain Rules

- Every query, job, solver artifact, report export, notification, and integration payload is scoped by `tenant_id`.
- Every clearing result must preserve `policy_snapshot`, `input_snapshot`, solver backend/version, solver artifact URIs, and explanation snapshots.
- Production `single_round_spot` success requires solver model/output artifacts; heuristic/recorded fixtures are allowed only for replay, benchmarks, or local diagnostics.
- Carrier-facing explanations must redact competitor bid details and expose only own-bid facts plus generalized binding constraints.
- Report templates bind only to frozen `report_exports.snapshot_json` and the §4.T tenant identity columns.
- Optional Notification Hub, Workflow Engine, and Webhook Engine failures are health/outbox events, not canonical state transitions.
